...
public class Plano {
	...
	public void leePlano(String f/*nombre de fichero*/){//hecho
		...
		String[] tmp = null;//para guardar cada caracter de la linea separado
...
/*arriba-izquierda*/	
	...
	public void borraDestino(Coordenadas c){
		//NEW PRACTICA2 para borrar de mi ArrayList los destinos visitados
		int pos = -1;
		
		for(int i = 0; i<destinos.size(); i++){
			if(destinos.get(i).getCoordenadas().getFila() == c.getFila() && destinos.get(i).getCoordenadas().getColumna() == c.getColumna())
				pos = i;
		}
		if(pos!=-1)
			destinos.remove(pos);
